        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          >
            <nav className="flex justify-center items-center gap-2 text-sm mb-6 text-white/80">
              ...
              <span className="text-white font-medium">Categorías de Tours</span>
            </nav>
            ...
            <p className="text-xl text-white/90 max-w-2xl mx-auto">
              Encuentra el tipo de experiencia que mejor se adapte a tu estilo de viaje
            </p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          ...
          >
            {categories.map((category, idx) => {
              const categoryTours = tours.filter(t => t.category === category.slug);
              const bgColors = [
                      ...
                      <div className="relative h-full flex flex-col justify-between p-8 text-white">
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-white/70">
                            Desde ${categoryTours.length > 0 ? Math.min(...categoryTours.map(t => t.price)) : 0}
                          </span>
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <span className="text-primary font-bold tracking-wider uppercase text-sm">Lo Más Popular</span>
            <h2 className="font-heading font-bold text-3xl md:text-4xl mt-2 text-gray-900">
              Tours Destacados por Categoría
            </h2>
          </div>

          {categories.slice(0, 3).map((category) => {
            const categoryTours = tours.filter(t => t.category === category.slug).slice(0, 3);
